...
{
    this->window = new sf::RenderWindow(sf::VideoMode(800, 600), "SpaceLords", sf::Style::Close | sf::Style::Titlebar);
...
{
    if (!this->shootBuffer.loadFromFile("Sounds/shoot.wav")) {
        std::cout << "ERROR::GAME::Failed to load shoot sound\n";
    }
    this->shootSound.setBuffer(this->shootBuffer);


    if (!this->bgMusic.openFromFile("Sounds/space.ogg")) {
